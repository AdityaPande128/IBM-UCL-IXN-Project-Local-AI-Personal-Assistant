...
self-extending AI assistant for personal computing. The focus is on exploring how a language model
running entirely on consumer hardware can be given practical agency over a user's own machine
through integration with OpenClaw, and on how such an assistant can overcome the reliability limits
of small models by acquiring new capabilities at runtime rather than shipping with a fixed feature
set. The project additionally explores voice and conversational interaction, capability-based
sandboxing of automatically generated code, and the evaluation methodology required to make claims
about a system whose behaviour is not deterministic.

The resulting application is named Jarvis.

#### 1.1.1 The Capability Accumulation Model

Conventional assistants are built around a fixed catalogue of abilities: the set of things the
assistant can do is decided by its developers and does not change in response to what any individual
user asks of it. A request outside that catalogue is refused, regardless of how ordinary it may be.
...
registers it permanently. The first time a task is requested it is slow, because a program must be
written and checked. Every subsequent time it is fast and deterministic, because the capability is
now ordinary code.

This matters because of a specific limitation. A language model small enough to run on a personal
laptop is not reliable enough to be trusted with a task on a single attempt. The premise of this
project is that per-invocation reliability is the wrong requirement. Reliability is required *once*,
at the moment a capability is created and independently verified; thereafter the capability is a
deterministic artefact and the model is no longer in the execution path at all. A system that is
...
OpenClaw is an open-source agent runtime. A language model on its own can only produce text.
OpenClaw gives it the means to act: it runs the model in a loop, lets it call tools such as the
shell, and reads descriptions of available capabilities from a `skills` directory. It supplies the
machinery for acting on a computer, which this project uses rather than reimplements.
...
reusable, parameterised capability, and building one is only worthwhile for tasks that recur.
Many requests are not like this — opening a particular link, asking what is running at this moment,
...
Assistants capable of operating a personal computer have advanced rapidly, but several problems
remain unaddressed for the individual user working with their own data.

**Privacy and Data Sensitivity:** The most useful tasks an assistant could perform involve precisely
the material a user is least willing to disclose — financial statements, medical correspondence,
academic work, personal archives. Cloud-hosted assistants require this material to be transmitted to
a third party, and the tasks with the greatest practical value are consequently the ones users are
most reluctant to delegate.

**Online Dependency and Cost:** Cloud assistants are unavailable without connectivity and are
metered per request. This discourages the routine, repetitive, low-value automation that would
benefit a user most, because each invocation carries a cost and a network round trip. Tasks that
would ideally run frequently, or on a schedule, or speculatively, become uneconomic.
...
in natural language from elsewhere, so the ability to act on one's own computer is tied to physical
proximity to it.

**Static Capability:** Assistants ship with a fixed repertoire. A request that falls outside it is
refused rather than satisfied, and the assistant does not become more capable in response to the
particular demands of the person using it. Users with recurring, individual needs are served no
better after months of use than on the first day.
...
**Privacy and Data Sensitivity Solution:** All inference is performed on the user's own machine
using locally hosted models, and user data is never sent to a third party. Because no external
service is involved, the assistant can be given access to exactly the sensitive material that makes
it useful.

A distinction is drawn between the code that processes a user's data and the code that transmits
results to the user. Automatically generated capabilities are denied network access at the
operating-system level: a generated skill reads and writes only within the scope of the request it
was given, and communicates its results by writing locally. Transmission is the exclusive
responsibility of the application's own daemon, which is developer-authored and reviewed. The
network boundary therefore falls between generated code and the transport layer. A generated skill
cannot send data anywhere, to the user or to anyone else, while the user still receives results on
whichever device they are using — including remotely. Exfiltration by automatically generated code
is prevented structurally rather than by trusting that code to behave.

**Online Dependency and Cost Solution:** Operating entirely offline, the assistant has no
per-request cost and no network dependency. This makes routine and repetitive automation economic in
a way that metered cloud invocation is not, and allows the system to remain useful without
connectivity.
...
**Static Capability Solution:** When no installed skill covers a request, the assistant writes one.
A generated skill is subjected to static analysis and executed against test cases inside a sandbox
...
more capable and more responsive with continued use.
...
one that fails is rejected and the failure recorded. Because verification performed by the model on
its own output is vulnerable to correlated error — a model that misunderstands a task will write
tests encoding the same misunderstanding — correctness is additionally assessed against criteria the
model never sees. Once admitted, a skill executes as deterministic code with no model involvement,
so the unreliability of generation does not propagate into use.
...
Finally, the system is to be evaluated rather than merely demonstrated: on the correctness of its
